...
class Sir:
    ...
    def __init__(self, p: Parameters):
        ...
        self.mitigation_model = p.mitigation_model
...
#             Similar problem on the other side. min_loss >= 14 won't work either
        ...
        for beta_t in self.beta_t:
            self.r_t.append(beta_t / gamma * susceptible)
        self.r_naught = self.beta / gamma * susceptible


        doubling_time_t = []
        for beta_t in self.beta_t:
            doubling_time_t.append( 1.0 / np.log2(beta_t * susceptible - gamma + 1))
        ...
        for d in self.doubling_time_t:
            self.daily_growth_rate_t.append(get_growth_rate(d))
